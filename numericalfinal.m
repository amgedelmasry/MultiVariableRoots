...
listbox=uilistbox(f,'Items',{'Gauss elimination','gauss seidle'},'position',[50 200 150 100]);
...
stringat="";
syms unknowns [n 1];
equations=["12*a + 3*b -5*c - 1"];
equations=[equations,"1*a + 5*b +3*c -28"];
equations=[equations,"3*a + 7*b  + 13*c - 76"];
solution=zeros(1,n);
solution2=[1,0,1];
app_error=0.008;
error_soln=[];
...
while(flag==1 && counter<maxiterations)
    for i=1:n
        ...
        err_soln(i)=(sum-solution2(i))/sum;
        solution2(i)=sum
    end
    flag=0
    for i=1:n
...
%     for i=1:n
%         label3=uilabel(f,'Text',strcat(char(96+i),':',num2str(solution2(i))),'FontSize',12,'Position',[20 750-(2*n+4+countit+(counter)*n+i)*20 1000 20]);
%     end
%     label4=uilabel(f,'Text',strcat('number of iterations',num2str(counter)),'FontSize',12,'Position',[20 750-(2*n+4+countit+(counter)*n+i+1)*20 1000 20]);
%     label4=uilabel(f,'Text',strcat('time taken to calculate:',num2str(time)),'FontSize',12,'Position',[20 750-(2*n+4+countit+(counter)*n+i+2)*20 1000 20]);
